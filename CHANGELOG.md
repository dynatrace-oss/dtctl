...
### Added
- **`dtctl get classic-pipelines-translation <logs|bizevents>`** — a read-only command that calls the OpenPipeline classic-pipelines translation endpoint (`GET /platform/openpipeline/v1/classic-pipelines/translate`) and returns the translated OpenPipeline configuration pipeline verbatim, as the reliable starting point for migrating Classic pipelines to OpenPipeline; the configuration scope is a positional argument (validated inline) and the three translation options are flags sent on every request so server defaults can be overridden (`--include-sample-data` [false], `--skip-disabled-rules` [true], `--skip-builtin-processing-rules` [false]); the translated pipeline document is treated as opaque and forwarded without interpretation — every output format emits the pipeline document directly (default and `-o json` as indented JSON, `-o yaml` as structured YAML) so it is applyable via the Settings API as-is, and `withWarning` (set when a processing rule's script could not be translated automatically and needs a manual rewrite) is surfaced on stderr in human mode and via the agent envelope in `-A` mode rather than in stdout. The underlying API is public but early-adopter and may change.
- **`dtctl query` can now output JSON Lines and Parquet via `-o jsonl` / `-o parquet`** — two new first-class output formats aimed at large result exports. `-o jsonl` emits one compact JSON object per line (newline-delimited JSON), encoded and written one record at a time rather than marshalling the whole result into a single buffer, and read natively by common local tooling (`dtctl query "fetch logs" -o jsonl`). `-o parquet` writes a columnar Apache Parquet file via the pure-Go `parquet-go` library (no CGO, so the static-binary/cross-compile story is preserved); its schema is derived from the DQL column types (`--include-types` is auto-enabled for this format), falling back to value-inference for columns the API does not type, and JSON-encoding nested/variant columns as a `complex` string column so a column is never silently dropped — an empty result still yields a valid, schema-bearing Parquet file rather than a zero-byte one (`dtctl query "fetch logs" --max-result-records 100000 -o parquet > logs.parquet`).
...
### Fixed
- **`dtctl get dashboard`/`notebook`/`launchpad` `-o yaml` and `-o json` now emit the document body instead of a list of raw byte values** — the 0.28.0 SDK split ([#239](https://github.com/dynatrace-oss/dtctl/pull/239)) introduced a CLI read-model type for documents whose `MarshalJSON`/`MarshalYAML` methods were never ported from the SDK type; YAML fell back to reflection and serialized `Content []byte` as an integer sequence (`- 123`, `- 34` …), while JSON silently dropped `content` entirely (the field is tagged `json:"-"`); `modificationInfo` timestamps were lost the same way; the CLI `Document` now delegates to the SDK marshaler, restoring structured `content` and `modificationInfo` in `json`/`yaml`/`toon` output (table/csv/wide were unaffected); affects 0.28.0–0.30.0; fixes [#281](https://github.com/dynatrace-oss/dtctl/pull/281)
- **`dtctl get analyzer -o yaml`, `dtctl exec analyzer -o yaml`, and `dtctl history -o yaml` no longer emit byte-value lists or leak display-only fields** — the same marshaling gap affected the analyzer (`Analyzer`, `AnalyzerDefinition`, `ExecuteResult`) and snapshot (`Snapshot`) CLI types, which had no `MarshalYAML` and relied on JSON struct tags that `yaml.v3` reflection ignores; for `AnalyzerDefinition`, the `input`/`output`/`analyzerCall` schemas (`json.RawMessage`) rendered as raw byte-integer sequences in YAML; across all four types YAML also lowercased keys, ignored `omitempty`, and surfaced `json:"-"` display fields (`categoryName`, `resultID`, `createdBy`, …) that JSON correctly hides; these types now implement `MarshalYAML` via a shared `format.YAMLNodeFromJSON` helper so YAML output is structurally identical to JSON; affects 0.28.0–0.30.0; fixes [#281](https://github.com/dynatrace-oss/dtctl/pull/281)
