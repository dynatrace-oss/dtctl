...
### Added
- **`dtctl get classic-pipelines-translation <logs|bizevents>`** — a read-only command that calls the OpenPipeline classic-pipelines translation endpoint (`GET /platform/openpipeline/v1/classic-pipelines/translate`) and returns the translated OpenPipeline configuration pipeline verbatim, as the reliable starting point for migrating Classic pipelines to OpenPipeline; the configuration scope is a positional argument (validated inline) and the three translation options are flags sent on every request so server defaults can be overridden (`--include-sample-data` [false], `--skip-disabled-rules` [true], `--skip-builtin-processing-rules` [false]); the translated pipeline document is treated as opaque and forwarded without interpretation — default output prints the pipeline as indented JSON, `-o json`/`-o yaml` emit the full `{value, withWarning}` result, and `withWarning` (set when a processing rule's script could not be translated automatically and needs a manual rewrite) is surfaced on stderr in human mode and via the agent envelope in `-A` mode. The underlying API is public but early-adopter and may change.
