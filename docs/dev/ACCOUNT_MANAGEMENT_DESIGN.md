...
| **Hardening** | `{id}.sprint.apps.dynatracelabs.com` | `api.sprint.dynatracelabs.com` (TBD) | `iam-hardening.dynatracelabs.com` | `sso-sprint.dynatracelabs.com` |
| **Development** | `{id}.dev.apps.dynatracelabs.com` | `api.dev.dynatracelabs.com` (TBD) | `iam-dev.dynatracelabs.com` | `sso-dev.dynatracelabs.com` |
...
```json
{
  "accounts": [
    {
      "account": {
        "id": "2a3b8834-cdbd-4f33-89e9-243074087162",
        "name": "Playground",
        "environments": [
          {
            "id": "wkf10640",
    ...
    {
      "account": {
        "id": "8556ea53-0ded-46f0-8809-6cd2809b9ac6",
...
### Decision 1: Config Schema -- No Changes

The `Context` struct is **unchanged**. No `account-uuid` or
`account-token-ref` fields are added to the config file. The account UUID
is auto-discovered at runtime via the IAM `access-info` endpoint
(see Decision 2), and the account-level token is managed transparently
in the keyring, keyed by the discovered account UUID.

The existing config remains exactly as it is:
...
The `Context` struct is unchanged:
...
For CI/CD or debugging, env vars can override the auto-discovered values:

| Variable | Description |
|----------|-------------|
| `DTCTL_ACCOUNT_UUID` | Override auto-discovered account UUID |
...
| **dtctl** (proposed) | Account UUID | Auto-discovered from environment via `access-info` | Not persisted (cached in keyring) |
...
1. `DTCTL_ACCOUNT_UUID` environment variable (runtime override)
2. **Auto-discover** via `access-info` endpoint:
...
# stderr: Using account "Dynatrace CA Capability" (8556ea53-...) for environment eqc04165
# ... group listing follows ...

# Inspect which account maps to your environment:
$ dtctl ctx discover-account
Found account "Dynatrace CA Capability" (8556ea53-...) for environment eqc04165
```
...
Accounts accessible to you:
  ...
  Playground                       2a3b8834-cdbd-4f33-89e9-243074087162    wkf10640 (Dynatrace Playground)
  Dynatrace CA Capability          8556ea53-0ded-46f0-8809-6cd2809b9ac6    eqc04165 (CA Production) ← current
  Dynatrace IAS                    9847750a-ebee-4537-ac0f-c07b62fb8d09    mjv28552 ovw30140
  Demo live                        7d63abff-b8fa-41f9-a1e8-2e7c8d4d77a2    guu84124 (Demo Live)

Current environment "eqc04165" belongs to account "Dynatrace CA Capability" (8556ea53-...)
```

This serves as a diagnostic tool -- users can see all accounts/environments
they have access to and verify the mapping is correct. The discovered UUID
is used at runtime; it is not persisted to the config file.
...
# Step 2: Auto-discover account UUID via access-info
# Step 3: PKCE flow with resource=urn:dtaccount:8556ea53-... (new)
...
&resource=urn:dtaccount:12345678-abcd-efgh-ijkl-123456789012
```
...
func accountBaseURLForTier(env Environment) string {
    ...
    case EnvironmentHard:
        return "https://api.sprint.dynatracelabs.com" // TBD: confirm
    case EnvironmentDev:
        return "https://api.dev.dynatracelabs.com"     // TBD: confirm
    default:
        return "https://api.dynatrace.com"
    }
}
...
Account API calls require a token with resource=urn:dtaccount:8556ea53-...
...
{
  "groups": ["541802b0-623c-4193-8728-036ed01d4eb4"],
  "boundaries": []  // optional
}
...
```json
{
  ...
  "email": "e2dbb9d3-...@service.sso.dynatrace.com",
  "name": "CI Pipeline",
  "surname": "SERVICE_IDENTITY",
  "description": "Event ingestion for CI",
  "createdAt": "2026-04-15T10:00:00Z",
  "groupUuid": "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
}
```
...
dtctl iam create policy \
  ...
  --level environment
# → Created policy "event-writer" (uuid: 0c621587-...)

# 2. Create a group
dtctl iam create group \
  --name "Event Writers" \
  --description "Service accounts for event ingestion"
# → Created group "Event Writers" (uuid: 541802b0-...)
...
# → Created service user "CI Pipeline" (uid: e2dbb9d3-...)
...
- Add `dtctl ctx discover-account` command
...
| Account Management API | Account URN | `urn:dtaccount:12345678-abcd-efgh-ijkl-123456789012` |
