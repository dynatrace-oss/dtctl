...
[dtctl docs](../README.md) / [Resources](README.md) / Lookup Tables
...
### Tab-delimited parse pattern

In addition to pipe-delimited input, a `parse-pattern` of `tab` handles tab-separated files:
...
### More enrichment patterns

Beyond error code enrichment, the `lookup` operator supports other common mappings:
