...
3. `.dtctl.yaml` in current directory or any parent directory (walks up to root)
...
For CI, point `DTCTL_CONFIG` at a config file you control. That file is treated as trusted, so environment variable references expand and inline tokens are accepted:

```yaml
# ci-config.yaml (not committed — generated by CI or supplied as a secret)
apiVersion: dtctl.io/v1
kind: Config
current-context: ci
contexts:
...
```bash
export DTCTL_CONFIG="$PWD/ci-config.yaml"
...
`DTCTL_CONTEXT` is session-local. To persist a context switch, use
`dtctl ctx <name>`.
