...
Examples:
  ...
  dtctl create settings -f alerting-profile.yaml --schema builtin:alerting.profile --scope environment

  # Create with template variables
  dtctl create settings -f settings.yaml --schema builtin:alerting.profile --scope environment --set name=prod

  # Dry run to preview
  dtctl create settings -f settings.yaml --schema builtin:alerting.profile --scope environment --dry-run
`,
...
func init() {
	...
	createLookupCmd.MarkFlagRequired("file")
