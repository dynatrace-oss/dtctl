...
func TestForbiddenIsTypedAndSurfacesReason(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			call: func(h *Handler) error {
				_, err := h.Create(AWSConnectionCreate{Value: Value{Name: "x", Type: TypeRoleBased}})
				return err
			},
		},
		{
			...
			permission:  "settings:objects:write",
			getSucceeds: true,
			call:        func(h *Handler) error { _, err := h.Update("obj-1", Value{Name: "x", Type: TypeRoleBased}); return err },
		},
		{
			name:       "delete",
			permission: "settings:objects:write",
		...
		t.Run(tt.name, func(t *testing.T) {
			h, server := newHandler(t, func(w http.ResponseWriter, r *http.Request) {
				...
				if tt.getSucceeds && r.Method == http.MethodGet {
					_ = json.NewEncoder(w).Encode(AWSConnection{ObjectID: "obj-1", SchemaID: SchemaID, SchemaVersion: "1", Value: Value{Name: "x", Type: TypeRoleBased}})
					return
				}
