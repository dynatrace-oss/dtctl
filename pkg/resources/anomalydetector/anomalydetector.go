...
func (h *Handler) Get(objectID string) (*AnomalyDetector, error) {
	...
	resp, err := req.Get(fmt.Sprintf("%s/%s", SettingsAPI, url.PathEscape(objectID)))
	if err != nil {
...
func (h *Handler) Delete(objectID string) error {
	resp, err := h.client.HTTP().R().Delete(fmt.Sprintf("%s/%s", SettingsAPI, url.PathEscape(objectID)))
	if err != nil {
